...
The modem is now explicitly tied to `GPIO 15` representing `RESET_N`. At 7 consecutive failed transmission cycles (with intervals defined by the user config), the ESP32 physically cycles this pin to trigger a hard EC200U reboot, wait 5 seconds, and cleanly re-initializes UART configuration.
